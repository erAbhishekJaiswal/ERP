...
//   const { subject_Id: paramid, date: paramDate } = useParams();
...
//   console.log("subjectId", subjectid, date)
  ...
  useEffect( () => {  
    ...
    if (selectedSubject && selectedDate) {
      fetchAttendance(selectedSubject, selectedDate); 
    } else {
      setAttendance(null);
    }
  ...
  const handleSaveAll = async () => {
    try {
      setLoading(true);
      await apiClient.put(
        `/api/attendance/${selectedSubject}/${selectedDate}`,
        {
    ...
    <div className="attendance-detail-container">
      ...
      !selectedSubject || !selectedDate ? (
          <div className="selection-prompt">
            <p>Please select a subject and date to view attendance details.</p>
          </div>
      ) :loading ? (
        ...
        <h1>Attendance Record</h1>
          ...
          <div className="action-buttons">
            {selectedSubject && (
              <Link to={`/attendance/subject/${selectedSubject}`}>
                <a className="back-btn">Back to Subject</a>
              </Link>
            )}
